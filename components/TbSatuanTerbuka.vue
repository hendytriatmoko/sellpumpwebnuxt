  ...
  <div>
    <section id="info-tb" class="mt-2">
      <v-row>
        ...
        <v-col cols="12" sm="3">
          <div
            :class="$vuetify.breakpoint.xs ? '' : 'fixed'"
            style="position: relative; top: 0; width: 290px"
          >
            <v-card rounded="lg">
              <div
                ...
                <v-col cols="5" sm="5">
                  <div v-if="start == false && end == false">
                    <center>
                      ...
                      <div class="pt-1 pb-1 d-inline-flex black--text">
                        <countdown
                  ...
                  <div v-if="start == true && end == false">
                    <center>
                      Berlangsung <br />
                      <div class="pt-1 pb-1 d-inline-flex teal white--text">
                        <countdown
              ...
              <div v-if="!guest">
                <div align="center" v-if="check == undefined">
                  ...
                  <div
                    ...
                    v-if="iklan.id_app_user == user.id"
                  >
                    Penjual dilarang melakukan penawaran pada iklan milik
                    sendiri
                  </div>
                </div>
                <div v-else>
                    ...
                    <center>
                      ...
                      >
                        <v-btn
                          ...
                          >Next Bid</v-btn
                        >
                        <v-btn v-else small outlined color="#20929D"
                          >Manual</v-btn
                        >
                  ...
                  <div v-if="toggle == 1">
                    ...
                    <div v-else>
                      <div v-if="reactivated">
                        <div class="d-flex align-center justify-space-around">
                          <v-btn small icon color="red" @click="minAuto()">
                            <v-icon>mdi-minus</v-icon>
                          </v-btn>
                          <h2 class="font-weight-bold" style="color: #4c4c4c">
                            Rp
                          ...
                          <v-btn small icon color="green" @click="addAuto()">
                            <v-icon>mdi-plus</v-icon>
                          </v-btn>
                        </div>
                        <div>
                        ...
                        <v-btn
                          ...
                          dark
                          class="mt-2"
                          style="margin-left: 19%; margin-right: 19%"
                          @click="autoBid"
                          v-if="isAuto == null"
                        >
                          Aktifkan Auto Tawar
            ...
            <v-card v-if="!guest">
              <div v-if="iklan.id_app_user == this.userid">
                ...
                <v-divider></v-divider>
                <div class="nullpenawaran" v-if="tawar_bersamas.length < 1">
                  <center>
                    <img
                      class="mt-4"
                      width="50px"
                ...
                <div v-else class="list_penawaran">
                  <div
                    v-for="(item, index) in tawar_bersamas"
                    :key="index"
                    class="d-flex align-center justify-space-around mt-3"
                  >
                    <div style="float: left">
                      <div
                        v-if="item.IdAppUser == userid"
                    ...
                    <div>
                      <div
                        class="list_harga rounded pl-1 pt-1 pr-1 pb-1"
                        style="
                          font-size: 12px;
                          background: #e9f4f5;
                          color: #20929d;
                        "
                        v-if="item.IdAppUser != user.id"
                      >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div
                          ...
                          "
                          v-if="
                            item.IdAppUser == user.id && item.IdTypeBid == 1
                          "
                        >
                          Rp {{ Number(item.Bid).toLocaleString('id-ID') }}
                        </div>
                        <div
                          ...
                          "
                          v-if="
                            item.IdAppUser == user.id && item.IdTypeBid == 2
                          "
                        >
                          Rp {{ Number(item.Bid).toLocaleString('id-ID') }}
                        </div>
            ...
            <br />
            <v-card v-if="!guest && start == true && end == true">
    ...
    <v-dialog v-model="pemenangpembeli" width="450">
      <v-card>
        <center>
          ...
          <div v-else>
            <v-icon color="orange" x-large> mdi-emoticon-sad </v-icon>
      ...
      <v-card>
        <center>
          ...
          <v-img
            ...
            height="80"
          ></v-img>
          <div v-if="tawar_bersamas.length > 0">
            <h3>Iklan Tawar Bersama Berakhir</h3>
            <p>
              Selamat Tawar bersama terbuka '{{ iklan.judul }}' <br />
              memiliki pemenang
            </p>
            <nuxt-link :to="'/message?id=' + tawar_bersamas[0].IdAppUser">
              <v-btn color="#20929D" class="white--text">
                Hubungi Pemenang
              </v-btn>
            </nuxt-link>
          ...
          <div v-else>
            <h3>Iklan Tawar Bersama Berakhir</h3>
            <p>
              Sayangnya Tawar bersama terbuka '{{ iklan.judul }}' <br />
      ...
      <v-card>
        ...
        <v-card-text>
          <div style="border: 2px solid #eeeeee">
            <div class="ml-2 my-2" style="color:color: #4C4C4C;font-size:20px">
              <b> Catatan</b>
            </div>
            <v-divider></v-divider>
        ...
        <label class="mx-4 my-4 pl-3">
          <input type="checkbox" v-model="catatankebijakan" :value="true" />
          Saya menyetujui catatan dan kebijakan dari penjual
        </label>
...
export default {
  ...
  methods: {
    ...
    tabes() {
      console.log('tabes', this.tawar_bersamas)
    },
    async getTB() {
      await this.$axios
    ...
    async gettawarbersama() {
      await this.$fire.firestore
        ...
        .onSnapshot((querySnapshot) => {
          ...
          this.congratsid = this.tawar_bersamas[0].IdAppUser
          this.congratsuniq = this.tawar_bersamas[0].IdUniq
          console.log('tawar bersama', this.tawar_bersamas)
        })
    },
    async listbid() {
      const topads = document.querySelectorAll('.list_harga')
      if (topads.length > 0) {
        topads[0].style.color = '#FFFFFF'
        topads[0].style.background = '#20929D'
      } else {
      }
    },
    async reviewAvg() {
  ...
  async created() {
    ...
    console.log('ini countdown detail 1', this.countdown)
    console.log('ini countdown detail 2', this.countdown2)
    console.log('ini auto', this.isAuto)
    console.log('ini route sementara', this.$route)
    console.log('ini id sementara nih', this.id)
    ...
    // }
    // , 1000)
    console.log('url', 'https://simotor.id/detail-iklan/' + this.iklan.judul.toLowerCase().replace(/ /g, '-').replace(/[/]/g,'-')+'-'+this.id,)
    console.log('title', this.iklan.judul)
    console.log('deskripsi', this.iklan.deskripsi)
    this.sharing = {
