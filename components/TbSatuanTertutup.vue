              ...
              <div v-else>
                ...
                <div v-else>
                  ...
                  <div v-else class="list_penawaran">
                    ...
                    >
                      <div style="float: left">
                        <div style="font-size: 14px">{{ usernama }}</div>
                        <div style="font-size: 10px">
                          {{
                            moment(item.created_at).format(
                              'DD-MMM-YYYY, hh:mm:ss'
    ...
    <v-dialog v-model="dialogKonfirmasi" width="460">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="#20929D" text outlined dark @click="dialogKonfirmasi = false">
            Tidak
          </v-btn>
        ...
        <center>
          ...
          <div v-else>
            <h3>Maaf anda kalah!</h3>
             <p class="mx-4">
              Sayang sekali Anda kalah dalam tawaran pada iklan '{{ iklan.judul }}', ayo ikuti iklan
              lainnya untuk memenangkan iklan Tawar Bersama.
            </p>
          </div>
...
export default {
    ...
    async getTB() {
      await this.$axios
        ...
        .then((response) => {
          ...
          console.log('cek opsi limit', this.iklan)
        })
        .catch((error) => {
          let responses = error.response.data
          console.log(responses.api_message)
  ...
  async created() {
    ...
    // console.log('post2', this.user.id)
    // console.log('post3', this.bid)

    if (!this.guest) {
      await this.checkTB()
    }

    //  start = return this.moment.utc(this.now).isAfter(this.iklan.tanggal_mulai)
    //  end = return this.moment.utc(this.now).isAfter(this.iklan.tanggal_selesai)
